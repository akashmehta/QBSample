...
/**
 * KerningTextView is a special {@link TextView} which allows adjusting the spacing between
 * characters in a piece of text AKA Kerning.
 * <p/>
 * You can use the {@code #setKerningFactor()} method to adjust the kerning programatically, but the
 * more common approach would be to use {@code ktv_spacing} attribute in xml.
 */
public class KerningTextView extends TextView {
    ...
    private void init(AttributeSet attrs, int defStyle) {
        ...
        TypedArray currentTypedArray = getContext().obtainStyledAttributes(attrs,
                                                                           R.styleable.KerningViews,
                                                                           0, defStyle);

        try {
            mKerningFactor = currentTypedArray.getFloat(R.styleable.KerningViews_kv_spacing,
                                                        Kerning.NO_KERNING);
            difference = currentTypedArray.getInteger(R.styleable.KerningViews_bold_str_diff,
                                                    0);
            endIndex = currentTypedArray.getInteger(R.styleable.KerningViews_end_bold_index,
                                                  0);
    ...
    private void applyKerning() {
        ...
        if (builder.toString().length() > 1) {
            ...
            finalText.setSpan(new StyleSpan(Typeface.BOLD),endIndex - (difference*2),
                              endIndex,
