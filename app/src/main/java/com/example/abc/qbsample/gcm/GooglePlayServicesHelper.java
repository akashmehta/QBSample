...
import com.example.abc.qbsample.Utils.DeviceUtils;
import com.example.abc.qbsample.Utils.SharedPrefsHelper;
import com.example.abc.qbsample.Utils.VersionUtils;
...
public class GooglePlayServicesHelper {
    ...
    public void registerForGcm(String senderId) {
        ...
        Log.e(TAG, "registerForGcm: __________________THE GCM REGISTRATION ID IS : "+gcmRegId);
    ...
    private void registerInGcmInBackground(String senderId) {
        System.out.println("....______________ sender id is ______________...."+senderId);
        new AsyncTask<String, Void, String>() {
            @Override
            protected String doInBackground(String... params) {
                try {
                    CoreApp coreApp = CoreApp.getInstance();
                    InstanceID instanceID = InstanceID.getInstance(coreApp);
                    try {
                        QBMessages.subscribeToPushNotificationsTask(instanceID.getToken(params[0], GoogleCloudMessaging.INSTANCE_ID_SCOPE, null),DeviceUtils.getDeviceUid(),QBEnvironment.DEVELOPMENT);
                        Log.w(TAG, "onSuccess: Device Registered successfully..........................");
            ...
            protected void onPostExecute(final String gcmRegId) {
                if (TextUtils.isEmpty(gcmRegId)) {
                    ...
                    final QBSubscription qbSubscription = new QBSubscription();
